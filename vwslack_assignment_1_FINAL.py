...
def import_data(test_input):
    participants = []
    with open(test_input, 'r') as file_name:
        csv_file = csv.reader(file_name)
        for line in csv_file:
            pokemon = [str(line[0]),str(line[1]),float(line[2]),float(line[3])]
...
# in away game, they get hit first
# fight every pokemon twice, one home, one away
# if you have 10 participants, there will be 18 games
# we will keep wins list
# initialize list with 0s
# for each participannt, do a nested group
    # in the inner loop, need to make sure that the index is + (because the pokemon cannot fight itself

# for i in range from 0 to 10
# for j in range # the index will

# the first pokemon will fight all pokemons first
# then move to the second, itll start at 3rd, then 4th,
# do both home and away games in the same loop
# if they have 
